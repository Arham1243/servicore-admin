...
export const useEmailTemplateStore = defineStore('EmailTemplateStore', () => {
    ...
    const changeStatus = (id, payload) => {
        return globalStore.actionWrapper(async () => {
            const res = await EmailTemplateService.changeStatus(id, payload);
            globalStore.showSuccess(
                'Email Template status updated',
    ...
    const previewTemplate = async (id, payload) => {
        const res = await EmailTemplateService.previewTemplate(id, payload);
        return res.data;
    };
