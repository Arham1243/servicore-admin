<script setup>
import { computed, onBeforeMount, ref } from 'vue';
import { useExpenseCategoryStore } from '@/modules/administration/stores';
import { useGlobalStore, useSessionStore } from '@/stores';
import { useExportTable } from '@/composables/useExportTable';
import { PaginationOptions, SortFilterOptions } from '@/config';
import { useHelpers } from '@/composables';

const { filterByPermission } = useHelpers();

const expenseCategoryStore = useExpenseCategoryStore();
const globalStore = useGlobalStore();
const sessionStore = useSessionStore();

const { exportTable } = useExportTable();
const pagination = new PaginationOptions();
const sortFilters = new SortFilterOptions();
const searchText = ref('');
const menu = ref();
const selectedItem = ref(null);
const loading = ref(false);
const items = ref([]);
const isEditMode = ref(false);
const quicbooksConnected = sessionStore?.myCompany?.qbo_connected;
const busy = ref(false);
const totalRecords = ref();
const showDialog = ref(false);
const deleteDialog = ref(false);
const changeStatusDialog = ref(false);
const loadingActiveDropdownItems = ref(false);
const exportMenu = ref(null);
const activeDropdownItems = ref([]);
const quickBooksList = ref([]);
const formData = ref({
    name: '',
    description: '',
    parent_id: null,
    code: '',
    system_link: '',
    default_markup: 0.0,
    force_uom_entry: false,
    uom_miles: null,
    uom_cost_rate: 0.0,
    uom_sale_price: 0.0,
    status: true
});

onBeforeMount(async () => {
    await getQuickBooksList();
    await getItems();
});

const menuItems = computed(() => {
    if (!selectedItem.value) return [];

    const allMenuItems = [
        {
            label: 'Edit',
            icon: 'pi pi-pencil',
            command: () => editItem(),
            permission: 'administration.edit'
        },
        {
            label: isItemActive.value ? 'Make Inactive' : 'Make Active',
            icon: isItemActive.value ? 'pi pi-times' : 'pi pi-check',
            command: () => showChangeStatusDialog(),
            permission: 'administration.edit'
        },
        {
            label: 'Delete',
            icon: 'pi pi-trash',
            command: () => showDeleteDialog(),
            permission: 'administration.delete'
        }
    ].filter(Boolean);

    return filterByPermission(allMenuItems);
});

const isItemActive = computed(() => {
    return selectedItem.value && selectedItem.value.status;
});

const exportMenuItems = [
    {
        icon: 'pi pi-file-excel',
        label: 'To Excel',
        command: () =>
            exportData({ table: 'expense_categories', format: 'excel' })
    },
    {
        icon: 'pi pi-file-pdf',
        label: 'To PDF',
        command: () =>
            exportData({ table: 'expense_categories', format: 'pdf' })
    }
];

const getQboAccountName = (id) => {
    if (!id || !quickBooksList.value) return null;
    const account = quickBooksList.value.find((acc) => acc.Id == id);
    return account ? account.Name : null;
};

const openDialog = (mode = 'add') => {
    isEditMode.value = mode === 'edit';
    showDialog.value = true;
    getActiveDropdownItems();
};

const closeDialog = () => {
    showDialog.value = false;
    resetForm();
};

const onShow = () => {
    resetForm();
};

const resetForm = () => {
    formData.value.name = '';
    formData.value.description = '';
    formData.value.parent_id = null;
    formData.value.code = '';
    formData.value.default_markup = 0.0;
    formData.value.force_uom_entry = false;
    formData.value.uom_miles = null;
    formData.value.uom_cost_rate = 0.01;
    formData.value.uom_sale_price = 0.01;
    formData.value.system_link = '';
    formData.value.status = true;
    globalStore.clearErrors();
};

const editItem = () => {
    resetForm();
    formData.value.name = selectedItem.value.name;
    formData.value.description = selectedItem.value.description;
    formData.value.parent_id = selectedItem.value.parent_id;
    formData.value.code = selectedItem.value.code;
    formData.value.default_markup = selectedItem.value.default_markup;
    formData.value.force_uom_entry = selectedItem.value.force_uom_entry;
    formData.value.uom_miles = selectedItem.value.uom_miles;
    formData.value.uom_cost_rate = selectedItem.value.uom_cost_rate;
    formData.value.uom_sale_price = selectedItem.value.uom_sale_price;
    formData.value.status = selectedItem.value.status;
    formData.value.system_link = selectedItem.value.system_link;
    openDialog('edit');
};

const showDeleteDialog = () => {
    deleteDialog.value = true;
};

const showChangeStatusDialog = () => {
    changeStatusDialog.value = true;
};

const showActions = (event, item) => {
    selectedItem.value = item;
    menu.value.toggle(event);
};

const showExportMenu = (event) => {
    exportMenu.value.toggle(event);
};

const exportData = async ({ table, format }) => {
    try {
        loading.value = true;
        const columns = ['display_name', 'code', 'status'];
        await exportTable({ table, format, columns });
    } finally {
        loading.value = false;
    }
};
const onSortChange = (event) => {
    pagination.resetPageParams();
    sortFilters.updateSortFilters(event);
    getItems();
};

const onPageChange = (event) => {
    pagination.updatePageParams(event);
    getItems();
};

const search = async () => {
    pagination.resetPageParams();
    sortFilters.updateSearch(searchText.value);
    getItems();
};

const getItems = async () => {
    try {
        loading.value = true;
        const params = {
            ...pagination.getPageParams()
        };
        const payload = sortFilters.getSortFilters(searchText.value);
        if (!payload.sort || payload.sort.length === 0) {
            payload.sort = [{ field: 'status', direction: 'desc' }];
        }
        const res = await expenseCategoryStore.search(payload, params);
        items.value = res.data;
        totalRecords.value = res.meta.total;
    } finally {
        loading.value = false;
    }
};

const getActiveDropdownItems = async (searchText = '') => {
    try {
        loadingActiveDropdownItems.value = true;
        const params = { limit: 300 };
        const payload = {
            search: {
                value: searchText
            },
            filters: [
                {
                    field: 'status',
                    operator: '=',
                    value: 1
                },
                isEditMode.value && selectedItem.value?.id
                    ? {
                          field: 'id',
                          operator: '!=',
                          value: selectedItem.value.id
                      }
                    : null
            ].filter(Boolean)
        };
        const res = await expenseCategoryStore.search(payload, params);
        activeDropdownItems.value = res.data;
    } finally {
        loadingActiveDropdownItems.value = false;
    }
};

const getQuickBooksList = async () => {
    if (!quicbooksConnected) return;
    try {
        loading.value = true;
        const res = await expenseCategoryStore.getQuickBooksList();
        quickBooksList.value = res.data;
    } finally {
        loading.value = false;
    }
};

const save = async () => {
    try {
        busy.value = true;
        if (isEditMode.value) {
            await expenseCategoryStore.update(
                selectedItem.value.id,
                formData.value
            );
        } else {
            await expenseCategoryStore.create(formData.value);
        }
        await getQuickBooksList();
        closeDialog();
        await getItems();
        selectedItem.value = {};
    } catch (error) {
        console.error(error);
    } finally {
        busy.value = false;
    }
};

const deleteItem = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await expenseCategoryStore.deleteItem(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
        loading.value = false;
    }
};

const changeStatus = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await expenseCategoryStore.changeStatus(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
        loading.value = false;
    }
};
</script>

<template>
    <TitleHeader>
        <template #title>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold">
                    Expense Accounts Setup
                </h1>
                <p class="mt-2 text-sm sm:text-base mx-auto sm:mx-0 mb-0">
                    These accounts identify Expenses.
                </p>
            </div>
        </template>
        <template #actions>
            <Button
                label="Export"
                icon="pi pi-download"
                variant="outlined"
                size="medium"
                @click="showExportMenu($event)"
                :disabled="items.length === 0 || loading"
            />

            <Menu ref="exportMenu" :model="exportMenuItems" :popup="true" />
            <Button
                v-if="$ability.can('administration.create')"
                label="Add New"
                @click="openDialog('add')"
            />
        </template>
    </TitleHeader>

    <Card class="py-3 px-2">
        <template #content>
            <BaseTable
                :value="items"
                :page="pagination.page"
                :rows="pagination.limit"
                :total-records="totalRecords"
                :loading="loading"
                @sort="onSortChange"
                @page="onPageChange"
            >
                <template #header>
                    <div class="flex justify-end mb-5">
                        <Search
                            v-model="searchText"
                            @search="search"
                            class="w-full sm:w-1/2 md:w-1/3 lg:w-1/3"
                        />
                    </div>
                </template>
                <template #empty> No expense accounts found. </template>
                <Column :sortable="true" field="display_name" header="Name" />

                <Column
                    field="system_link"
                    header="System Link"
                    v-if="quicbooksConnected"
                >
                    <template #body="{ data }">
                        <span
                            v-if="
                                quicbooksConnected &&
                                data.system_link &&
                                data.system_link !== 'NC'
                            "
                        >
                            {{ getQboAccountName(data.system_link) }}
                        </span>
                    </template>
                </Column>

                <Column
                    class="w-[25rem]"
                    :sortable="true"
                    field="code"
                    header="Code"
                />

                <Column header="Status" :sortable="true" field="status">
                    <template #body="{ data }">
                        <StatusTag
                            :status="data.status ? 'active' : 'inactive'"
                        />
                    </template>
                </Column>

                <Column
                    v-if="
                        $ability.can('administration.edit') ||
                        $ability.can('administration.delete')
                    "
                    header="Actions"
                    class="flex justify-end"
                >
                    <template #body="{ data }">
                        <Button
                            class="!px-3 !py-2"
                            label="Actions"
                            variant="outlined"
                            iconPos="right"
                            icon="pi pi-chevron-down"
                            size="small"
                            @click="showActions($event, data)"
                        />

                        <Menu
                            ref="menu"
                            id="overlay_menu"
                            :model="menuItems"
                            :popup="true"
                        />
                    </template>
                </Column>
            </BaseTable>
        </template>
    </Card>

    <BaseDialog
        v-if="
            $ability.can('administration.edit') ||
            $ability.can('administration.create')
        "
        v-model:visible="showDialog"
        @update:visible="onShow"
        :busy="busy"
        :isEditMode="isEditMode"
        :header="isEditMode ? 'Edit Expense Account' : 'New Expense Account'"
        :confirmLabel="isEditMode ? 'Update' : 'Save'"
        :formData="formData"
        :initialData="isEditMode ? selectedItem : null"
        :enableDirtyCheck="true"
        :confirmBeforeSave="isEditMode ? true : false"
        @cancel="closeDialog"
        @confirm="save"
    >
        <div class="mb-3 col-span-12">
            <label class="block required mb-2" for="name">Name</label>
            <InputField
                variant="text"
                id="name"
                v-model="formData.name"
                class="w-full"
                @keyup.enter="save"
                :disabled="busy"
            />
        </div>

        <div class="mb-3 col-span-12">
            <label class="block mb-2" for="parent_id">Parent Account</label>
            <ApiDropdown
                showClear
                filter
                placeholder="Select"
                :loading="loadingActiveDropdownItems"
                @search="(s) => getActiveDropdownItems(s)"
                :options="activeDropdownItems"
                optionLabel="name"
                optionValue="id"
                id="parent_id"
                v-model="formData.parent_id"
                class="w-full"
                :disabled="busy || loadingActiveDropdownItems"
            />
        </div>

        <div class="mb-3 col-span-12 sm:col-span-12">
            <label class="block mb-2" for="code">Code</label>
            <InputField
                variant="text"
                id="code"
                v-model="formData.code"
                class="w-full"
                @keyup.enter="save"
                :disabled="busy"
            />
        </div>

        <div class="mb-3 col-span-12">
            <label class="block mb-2" for="description">Description</label>
            <InputField
                variant="textarea"
                id="description"
                v-model="formData.description"
                class="w-full"
                :disabled="busy"
            />
        </div>

        <div class="mb-3 col-span-12">
            <label class="block mb-2" for="system_link">System Link </label>
            <InputField
                v-if="quicbooksConnected"
                showClear
                filter
                id="system_link"
                v-model="formData.system_link"
                :loading="loading"
                variant="dropdown"
                :options="quickBooksList"
                optionLabel="Name"
                optionValue="Id"
                placeholder="Select"
                class="w-full"
                :disabled="busy || loading"
            />
            <InputField
                v-else
                variant="text"
                id="system_link"
                value="NC"
                class="w-full"
                @keyup.enter="save"
                :disabled="true"
            />
            <Message
                v-if="quicbooksConnected"
                severity="warn"
                icon="pi pi-info-circle"
                class="block !mt-4"
            >
                Linking account will result in automatic updates in QuickBooks
                using ServiCore definitions.</Message
            >
        </div>

        <div class="mb-3 col-span-12">
            <label class="block mb-2" for="default_markup"
                >Default Markup</label
            >
            <InputField
                variant="number"
                v-model="formData.default_markup"
                :min="0"
                :max="100"
                :showButtons="false"
                suffix="%"
                :disabled="busy"
                :maxFractionDigits="2"
                :minFractionDigits="2"
                :useGrouping="false"
                id="default_markup"
                inputClass="w-full"
                class="w-full"
                @keyup.enter="save"
            />
        </div>

        <div class="col-span-12">
            <div class="mb-3">
                <div class="flex items-center gap-2">
                    <InputField
                        binary
                        inputId="force_uom_entry"
                        variant="checkbox"
                        v-model="formData.force_uom_entry"
                        :disabled="busy"
                    />
                    <label for="force_uom_entry" class="cursor-pointer">
                        Enter number of units instead of total cost
                    </label>
                </div>

                <div v-if="formData.force_uom_entry" class="mt-7">
                    <label class="block text-xl font-bold mb-4"
                        >Units of Measure</label
                    >
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12 sm:col-span-4">
                            <label class="block mb-2" for="uom_miles"
                                >UoM (e.g. Miles)</label
                            >
                            <InputField
                                variant="text"
                                id="uom_miles"
                                v-model="formData.uom_miles"
                                class="w-full"
                                :disabled="busy"
                                @keyup.enter="save"
                            />
                        </div>
                        <div class="col-span-12 sm:col-span-4">
                            <label class="block mb-2" for="uom_cost_rate"
                                >Cost/Rate</label
                            >
                            <InputField
                                variant="number"
                                id="uom_cost_rate"
                                prefix="$"
                                v-model="formData.uom_cost_rate"
                                mode="decimal"
                                :min="0.01"
                                :max="9999.99"
                                :maxFractionDigits="4"
                                :minFractionDigits="4"
                                inputClass="w-full"
                                :disabled="busy"
                                @keyup.enter="save"
                            />
                        </div>
                        <div class="col-span-12 sm:col-span-4">
                            <label class="block mb-2" for="uom_sale_price"
                                >Sale/Price</label
                            >
                            <InputField
                                variant="number"
                                id="uom_sale_price"
                                prefix="$"
                                v-model="formData.uom_sale_price"
                                mode="decimal"
                                :min="0.01"
                                :max="9999.99"
                                :maxFractionDigits="4"
                                :minFractionDigits="4"
                                inputClass="w-full"
                                :disabled="busy"
                                @keyup.enter="save"
                            />
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="mb-3 col-span-12">
            <label class="block mb-3">Status</label>
            <div class="flex items-center gap-3">
                <InputField
                    inputId="status"
                    variant="switch"
                    v-model="formData.status"
                    :disabled="busy"
                />
                <label class="cursor-pointer mt-1" for="status">{{
                    formData.status ? 'Active' : 'Inactive'
                }}</label>
            </div>
        </div>
    </BaseDialog>

    <Confirmation
        v-if="$ability.can('administration.delete')"
        v-model="deleteDialog"
        variant="danger"
        header="Delete Expense Account"
        content="Are you sure you want to delete this expense account?"
        @confirm="deleteItem"
    />

    <Confirmation
        v-if="$ability.can('administration.edit')"
        v-model="changeStatusDialog"
        variant="danger"
        :header="isItemActive ? 'Make Inactive' : 'Make Active'"
        :content="`Are you sure you want to make this expense account ${isItemActive ? 'inactive' : 'active'}?`"
        :confirmButtonText="isItemActive ? 'Make Inactive' : 'Make Active'"
        @confirm="changeStatus"
    />
</template>
