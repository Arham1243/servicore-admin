...
const onRowReorder = async (event) => {
    ...
    try {
        ...
        await projectTypeStore.changeOrder(movedItemId, payload);
        await getItems();
    } catch (error) {
        console.error(error);
    } finally {
...
const getItems = async () => {
    try {
        ...
        const res = await projectTypeStore.search(payload, params);
...
const getMaxOrder = async () => {
    try {
        ...
        const res = await projectTypeStore.getMaxOrder();
...
const save = async () => {
    try {
        busy.value = true;
        if (isEditMode.value) {
            await projectTypeStore.update(
                selectedItem.value.id,
                formData.value
            );
        } else {
            await projectTypeStore.create(formData.value);
        }
        closeDialog();
        await getItems();
        selectedItem.value = {};
...
const deleteItem = async () => {
    try {
        ...
        if (selectedItem.value) {
            await projectTypeStore.deleteItem(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
const changeStatus = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await projectTypeStore.changeStatus(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
<template>
    <TitleHeader>
        <template #title>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold">Project Types</h1>
                <p class="mt-2 text-sm sm:text-base mx-auto sm:mx-0 mb-0">
                    These types will be used in defining projects.
                </p>
            </div>
        </template>
        <template #actions>
    ...
    <Card class="py-3 px-2">
        <template #content>
            ...
            >
                ...
                <template #empty> No project types found. </template>
                <Column
                    class="w-[3rem]"
                    :sortable="true"
                    field="order"
                ...
                >
                    <template #body="{ data }">
                        <span
                            ...
                            "
                        >
                            {{ truncate(data.description, { length: 30 }) }}
                        </span>
                    </template>
                </Column>
    ...
    <BaseDialog
        ...
        :header="isEditMode ? 'Edit Project Type' : 'New Project Type'"
    ...
    >
        ...
        <div class="mb-3 col-span-12">
            <label class="block mb-3" for="description"
                >Project Type Description</label
            >
            <InputField
                variant="textarea"
                id="description"
    ...
    <Confirmation
        ...
        content="Are you sure you want to delete this project type?"
        @confirm="deleteItem"
    />
    ...
    <Confirmation
        ...
        :content="`Are you sure you want to make this project type ${isItemActive ? 'inactive' : 'active'}?`"
