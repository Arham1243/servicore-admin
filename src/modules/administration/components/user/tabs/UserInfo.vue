...
const customers = ref([]);
const loadingQuickBooksList = ref(false);
...
const getQuickBooksList = async () => {
    ...
    try {
        loadingQuickBooksList.value = true;
        const res = await userStore.getQuickBooksList();
        quickBooksList.value = res.data;
    } finally {
        loadingQuickBooksList.value = false;
    }
};
...
const getUsers = async (key, searchText = '') => {
    ...
    try {
        ...
        list.value = res.data?.map((u) => ({ id: u.id, name: u.name })) || [];
    } finally {
        loading.value = false;
    }
};
...
<template>
    <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12">
            <label class="block mb-2">System Link</label>
            <InputField
                ...
                placeholder="Select"
                class="w-full"
                :disabled="busy || loadingQuickBooksList"
            />
            <InputField
                v-else
                variant="text"
