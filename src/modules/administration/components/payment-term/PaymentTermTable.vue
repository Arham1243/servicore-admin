...
watch(
    ...
    (val) => {
        if (!val) {
            formData.value.system_link = null;
        }
    }
);
...
const editItem = () => {
    ...
    formData.value.days_out = selectedItem.value.days_out;
...
const getItems = async () => {
    try {
        ...
        const res = await paymentTermStore.search(payload, params);
...
const getQuickBooksList = async () => {
    ...
    try {
        loading.value = true;
        const res = await paymentTermStore.getQuickBooksList();
...
const save = async () => {
    try {
        ...
        busy.value = true;
        if (isEditMode.value) {
            await paymentTermStore.update(
                selectedItem.value.id,
                formData.value
            );
        } else {
            await paymentTermStore.create(formData.value);
        }
        await getQuickBooksList();
        closeDialog();
        await getItems();
...
const deleteItem = async () => {
    try {
        ...
        if (selectedItem.value) {
            await paymentTermStore.deleteItem(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
const changeStatus = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await paymentTermStore.changeStatus(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
const makeDefault = async () => {
    try {
        loading.value = true;
        if (selectedItem.value) {
            await paymentTermStore.makeDefault(selectedItem.value.id);
        }
        await getItems();
        selectedItem.value = {};
    } finally {
...
<template>
    <TitleHeader>
        <template #title>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold">Payment Terms</h1>
                <p class="mt-2 text-sm sm:text-base mx-auto sm:mx-0 mb-0">
                    These payment terms are used to define customers and
                    invoices.
                </p>
            </div>
        </template>
    ...
    <Card class="py-3 px-2">
        <template #content>
            ...
            >
                ...
                <template #empty> No payment terms found. </template>
                <Column
                    :sortable="true"
                    field="name"
                ...
                <Column :sortable="true" field="days_out" header="Days" />
    ...
    <BaseDialog
        ...
        :header="isEditMode ? 'Edit Payment Term' : 'New Payment Term'"
    ...
    >
        <div class="mb-3 col-span-12">
            <label class="block required mb-3" for="name"
                >Name (label on your invoices)</label
            >
            <InputField
                variant="text"
                id="name"
        ...
        <div class="mb-3 col-span-12">
            <label class="block mb-3" for="days_out"
                >Days Out (eg - days until payment is due)</label
            >
            <InputField
                variant="number"
                id="days_out"
                v-model.number="formData.days_out"
                class="w-full"
                @keyup.enter="save"
                :useGrouping="false"
                inputClass="w-full"
    ...
    <Confirmation
        ...
        :content="`Are you sure you want to make this payment term ${isItemActive ? 'inactive' : 'active'}?`"
    ...
    <Confirmation
        ...
        :content="`Are you sure you want to make this payment term default?`"
        @confirm="makeDefault"
    />
</template>
